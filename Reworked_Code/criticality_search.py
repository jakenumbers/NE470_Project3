...
def crit_search(core_mat, nodes_density: float, width_low: float, width_high: float, progressprint:bool,
    ...
    regs = Regions(Region("Bare Core", w=width_low, n=wl_num_nodes, mat=core_mat))
    mesh = Mesh(regs)
    A, F = build_matrices(mesh)
    wl_res = solve_keff(A, F)
    
    wh_num_nodes = int(width_high * nodes_density)
    regs = Regions(Region("Bare Core", w=width_high, n=wh_num_nodes, mat=core_mat))
    mesh = Mesh(regs)
    A, F = build_matrices(mesh)
    ...
    for it in range(max_iter):
        ...
        regs = Regions(Region("Bare Core",w=width,n=num_nodes,mat=core_mat))
        mesh = Mesh(regs)
        A, F = build_matrices(mesh)
